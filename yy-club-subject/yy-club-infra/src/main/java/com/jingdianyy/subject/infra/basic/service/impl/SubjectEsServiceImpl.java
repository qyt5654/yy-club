...
import com.jingdianyy.subject.infra.basic.entity.EsSubjectFields;
import com.jingdianyy.subject.infra.basic.entity.SubjectInfoEs;
import com.jingdianyy.subject.infra.basic.es.EsIndexInfo;
import com.jingdianyy.subject.infra.basic.es.EsRestClient;
import com.jingdianyy.subject.infra.basic.es.EsSearchRequest;
import com.jingdianyy.subject.infra.basic.es.EsSourceData;
...
public class SubjectEsServiceImpl implements SubjectEsService {
    ...
    private EsIndexInfo getEsIndexInfo(){
        ...
        esIndexInfo.setClusterName("b1f11cf36d5a");
    ...
    private SubjectInfoEs convertResult(SearchHit hit) {
        Map<String, Object> stringAsMap = hit.getSourceAsMap();
        if(CollectionUtils.isEmpty(stringAsMap)){
            return null;
        }
        SubjectInfoEs result = new SubjectInfoEs();
        result.setSubjectId(MapUtils.getLong(stringAsMap, EsSubjectFields.SUBJECT_ID));
        result.setSubjectName(MapUtils.getString(stringAsMap, EsSubjectFields.SUBJECT_NAME));
        result.setSubjectAnswer(MapUtils.getString(stringAsMap, EsSubjectFields.SUBJECT_ANSWER));
        result.setDocId(MapUtils.getLong(stringAsMap, EsSubjectFields.DOC_ID));
        result.setSubjectType(MapUtils.getInteger(stringAsMap, EsSubjectFields.SUBJECT_TYPE));
